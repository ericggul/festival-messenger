...
const Component = ({ top, left, idx, angle, size, color }: any) => {
  ...
  const leftInterval = useMemo(() => getRandom(3, 10), []);
  const bottomInterval = useMemo(() => getRandom(3, 10), []);
  useEffect(() => {
    if (boxRef && boxRef.current) {
      const bundles = "loading".repeat(15).split("");
...
function Loading() {
  ...
  return (
    <S.Container>
      {Array.from({ length: 30 }, () => getRandom(windowWidth * 0.1, windowWidth * 0.35)).map((size, i) => (
