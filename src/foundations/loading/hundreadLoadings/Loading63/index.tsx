...
const getRandomColor = () => `hsl(${getRandom(150, 250)}, ${getRandom(90, 100)}%, ${getRandom(80, 90)}%)`;

const Component = ({ top, left, idx, angle, size, color }: any) => {
  const boxRef = useRef<any>(null);

  const angleInterval = useMemo(() => getRandom(5, 10), []);

  useEffect(() => {
    if (boxRef && boxRef.current) {
      const bundles = "loading".repeat(10).split("");
